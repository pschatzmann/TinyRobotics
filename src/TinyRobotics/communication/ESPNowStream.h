...
#include "TinyRobotics/utils/BaseStream.h"
#include "TinyRobotics/concurrency/RTOS/BufferRTOS.h"
...
struct ESPNowStreamConfig {
  ...
  bool use_send_ack = false;  // we wait for
...
class ESPNowStream : public BaseStream {
 public:
  ...
  bool addPeer(const char* address) {
    if (strcmp(address, cfg.mac_address)==0) {
