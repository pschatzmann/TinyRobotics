...
/**
 * @brief This class represents a time duration with a specific unit (seconds,
 * milliseconds, microseconds, minutes, or hours). It provides methods to set
 * and get the time in different units, as well as handle unit conversion when
 * retrieving the time in a desired unit. The internal representation of the
 * time is stored in the unit specified at construction, and all operations
 * ensure that the time can be accurately converted between supported units.
 ...
 * getTime method allows for easy retrieval of the time in the desired unit,
 * while the setTime method allows for updating the time measurement with a new
